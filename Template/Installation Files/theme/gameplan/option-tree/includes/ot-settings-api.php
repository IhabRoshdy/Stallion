          ...
          if ( ! isset( $page['hidden_page'] ) && $screen->id == $this->page_hook[$page['id']] ) {
            ...
            if ( $show_buttons ) {
              ...
              echo '<div id="option-tree-settings-api">';
				...
				echo '<span class="option-tree-ui-button left version">ver. ' . $current_theme->get('Version') . '</span>';
                  
                if ( $show_buttons )
                  echo '<button class="option-tree-ui-button button button-primary right">' . $page['button_text'] . '</button>';
			  
			    echo '<span class="option-tree-ui-button button button-secondary right document"><a href="http://doc.cactusthemes.com/gameplan/">Documentation</a></span>';
				echo '<span class="option-tree-ui-button button button-secondary right support"><a href="http://ticket.cactusthemes.com">Support Forum</a></span>';
				
                if(ct_check_for_update(PARENT_THEME) == 1){
				  echo '<button class="button right" style="margin:8px" name="ct_update_theme" type="submit" value="ct_update_theme" onclick="jQuery(\'#option-tree-settings-api\').attr(\'action\',\'themes.php?page=ot-theme-options&ct_update_theme=true\')">' . esc_htm__('New Version Found! Update Theme?','cactusthemes') . '</button>';
				}  
              ...
              if ( $show_buttons ) {
				...
				echo '<a href="' . str_replace( '&settings-updated=true', '', $_SERVER["REQUEST_URI"] ) .'&import_data=true" class="button red" style="left:30px;position: relative;top: -40px;height: 30px; padding-top: 3px" onclick="return confirm(\'' . __('We will import sample data. Make sure you have all required plugins installed! (The process will take few minutes. Stay calm!)','cactusthemes').'\');">' . __('Import Sample Data','cactusthemes') . '</a>';
