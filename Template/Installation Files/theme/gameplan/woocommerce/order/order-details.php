...
<table class="shop_table order_details">
	...
	<?php
				...
				if ( $has_refund && $key === 'order_total' ) {
					...
					if ( wc_tax_enabled() && 'incl' == $order->tax_display_cart ) {
						...
						if ( ! empty( $tax_del_array ) ) {
							$refunded_tax_del .= ' ' . sprintf( esc_html__( '(Includes %s)', 'woocommerce' ), implode( ', ', $tax_del_array ) );
						}

						if ( ! empty( $tax_ins_array ) ) {
							$refunded_tax_ins .= ' ' . sprintf( esc_html__( '(Includes %s)', 'woocommerce' ), implode( ', ', $tax_ins_array ) );
						}
					}
		...
		if ( $has_refund ) { ?>
			<tr>
				<th scope="row"><?php esc_html_e( 'Refunded:', 'woocommerce' ); ?></th>
...
<?php endif; ?>
		...
		<address><p>
			<?php
				if ( ! $order->get_formatted_billing_address() ) esc_html_e( 'N/A', 'woocommerce' ); else echo $order->get_formatted_billing_address();
			?>
		</p></address>
...
<?php if ( get_option( 'woocommerce_ship_to_billing_address_only' ) === 'no' && get_option( 'woocommerce_calc_shipping' ) !== 'no' ) : ?>
	...
	<div class="col-2">
		...
		<address><p>
			<?php
				if ( ! $order->get_formatted_shipping_address() ) esc_html_e( 'N/A', 'woocommerce' ); else echo $order->get_formatted_shipping_address();
			?>
		</p></address>
